...
**Approach:** - Two-step approach: 1) independent analyses for each water body, then take results on to analysis of the entire area (European level)\
- Motivation: - analyses should both take into account the information at the site level (within water bodies), but at the same time, water bodies are the proper unit for statistical analyses at the European level\
- analyses of raw data (both within one water body, or at the European level) are susceptible to chnages in the sampling scheme. For instance, if sampling one year an onwards is extended to include sites with higher NO3 levels, this - Use model s(year) (w/ bootstrap?) for each water body - Then use use predictions from those to model s(x,y) + s(year)\
- Use ordinary spatial thin-plate smooth, i.e. default version of s(x,y) instead of soap-film smoother - Using data for all Europe
...
saveRDS(pdata_for_gam_allyrs, "Data/120_pdata_for_gam_allyrs.rds")

# if you want to read this directly:
# pdata_for_gam_allyrs <- readRDS("Data/120_pdata_for_gam_allyrs.rds")
...
# For dat_test4
dat_test4$fit <- predict(m2, newdata = dat_test4)
dat_test4_mean <- dat_test4 %>%
  group_by(year) %>% summarize(fit = mean(fit))

ggplot(dat_test, aes(x = year, y = log_no3)) +
...
m2 <- gam(log_no3 ~ s(year, k = 3) + s(site_f, bs = "re"), data = dat_test)  
...
run_gam_2 <- function(wb, sitegroups, years){
  ...
  k_max <- max(k_max, 3)
  if (length(unique(data_sel$site)) > 1){
    mod <- gam(log_no3 ~ s(year, k = k_max) + s(site_f, bs = "re"), 
        data = data_sel)
    result <- get_predictions(model = mod, data = data_sel)
  } else {
    mod <- gam(log_no3 ~ s(year, k = k_max), data = data_sel)
    result <- get_predictions(model = mod, data = data_sel)
  }
  result
}
...
dat %>%
  summarize(no3 = 10^mean(log_no3))

dat %>%
  summarize(no3 = mean(10^log_no3),
            no3_med = median(10^log_no3))
...
if (rerun_gam_fitting1){
  ...
  get_wb_means <- function(result_orig, metadata_waterbody){
    ...
    bind_rows(result$result[ok]) %>%
      ...
      summarize(log_no3 = mean(fit), .groups = "drop") %>%
      left_join(metadata_waterbody %>% select(waterbody, x, y, sitegroup),
                by = "waterbody")
  }
  ...
  saveRDS(dat_wb_bysite, "Data/120_dat_wb_bysite.rds")
  saveRDS(dat_wb_means, "Data/120_dat_wb_means.rds")
  # saveRDS(res_orig, "Data/120_res_orig.rds")
  
}

dat_wb_means <- readRDS("Data/120_dat_wb_means.rds")  
...
# if(TRUE) {
  ...
  tic()
  mod1 <- gam(log_no3 ~ s(x, y, year, k=600), data = dat_wb_means[[1]])
  saveRDS(mod1, "Data/120_mod1.rds")
  mod2 <- gam(log_no3 ~ s(x, y, year, k=600), data = dat_wb_means[[2]])
  saveRDS(mod2, "Data/120_mod2.rds")
  mod3 <- gam(log_no3 ~ s(x, y, year, k=600), data = dat_wb_means[[3]])
  saveRDS(mod3, "Data/120_mod3.rds")
  mod4 <- gam(log_no3 ~ s(x, y, year, k=600), data = dat_wb_means[[4]])
  saveRDS(mod4, "Data/120_mod4.rds")
  toc() # 122 sec  

}

# Read saved data  
mod1 <- readRDS("Data/120_mod1.rds")
mod2 <- readRDS("Data/120_mod2.rds")
mod3 <- readRDS("Data/120_mod3.rds")
mod4 <- readRDS("Data/120_mod4.rds")
...
-   For the input data (the smoothed WB data 'pred_by_wb')\
-   Not including years with no data for the given WB
    -   In other words: only using existing combinations of year and water body\
-   NOTE: for that reason, heavily affected by WBs coming in later in the process

```{r}

# Get predictions from spatial model   
tic()
pred_by_wb <- purrr::map(mod, predict)
toc()

# Add predictions to data  
dat_wb_means <- purrr::map2(dat_wb_means, pred_by_wb, ~mutate(.x, modelfit = .y))
...
# Change of predicted values      

get_modelfit_byyear <- function(data_with_fit){
  yearmod <- gam(modelfit ~ s(year, k = 8), data = data_with_fit)  
  # yearmod <- gam(no3_fit ~ s(year, k = 8), data = data_with_fit %>% mutate(no3_fit = 10^modelfit))  
  yearmod_pred = data.frame(year = seq(min(data_with_fit$year), 2020))
  fit_object <- predict(yearmod, newdata = yearmod_pred, se.fit = TRUE)
  yearmod_pred %>%
    mutate(fit = fit_object$fit,
           se.fit = fit_object$se.fit)
}

dat_year_fit <- map_dfr(dat_wb_means, get_modelfit_byyear, .id = "Model run")

ggplot(dat_year_fit, aes(year, fit, group = `Model run`)) +
  geom_ribbon(aes(ymin = fit - 2*se.fit, ymax = fit + 2*se.fit), fill = "grey50", alpha = 0.3) +
  geom_line(aes(color = `Model run`)) +
  theme_bw()

ggplot(dat_year_fit, aes(year, fit, group = `Model run`)) +
  geom_ribbon(aes(ymin = fit - 2*se.fit, ymax = fit + 2*se.fit, fill = `Model run`), alpha = 0.3) +
  geom_line(aes(color = `Model run`)) +
  theme_bw()
...
m1 <- gam(modelfit ~ s(year, k = 8), data = dat_wb_means[[1]])  
plot(m1)  

ggplot(get_modelfit_byyear(dat_wb_means[[1]]), aes(year, fit)) +
  geom_ribbon(aes(ymin = fit - 2*se.fit, ymax = fit + 2*se.fit), fill = "lightblue") +
  geom_line()
...
get_fit_allyears <- function(data, model){
  ...
  newdata = data.frame(
    ...
    left_join(df_wb %>% select(waterbody, x, y)) 
...
#### Plot year effect, back-transformed to ordinary scale

```{r}

ggplot(dat_year_fit, aes(year, yearfit_back, group = modelrun)) +
  geom_ribbon(aes(ymin = yearfit_back_lo, ymax = yearfit_back_up), 
              fill = "grey50", alpha = 0.3) +
  geom_line(aes(color = modelrun)) +
  labs(y = "NO3 conc.") +
  theme_bw()
...
#### Plot year effect, back-transformed before gam
...
## 9. GAM, time + random effect for waterbody
...
# if(TRUE) {
  ...
  tic()
  df <- dat_wb_means[[1]] %>% mutate(waterbody_f = factor(waterbody))
  modr1 <- gam(log_no3 ~ s(year, k = 8) + s(waterbody_f, bs = "re"), data = df)
  df <- dat_wb_means[[2]] %>% mutate(waterbody_f = factor(waterbody))
  modr2 <- gam(log_no3 ~ s(year, k = 8) + s(waterbody_f, bs = "re"), data = df)
  df <- dat_wb_means[[3]] %>% mutate(waterbody_f = factor(waterbody))
  modr3 <- gam(log_no3 ~ s(year, k = 8) + s(waterbody_f, bs = "re"), data = df)
  df <- dat_wb_means[[4]] %>% mutate(waterbody_f = factor(waterbody))
  modr4 <- gam(log_no3 ~ s(year, k = 8) + s(waterbody_f, bs = "re"), data = df)
  toc() # 370 sec  

  modr <- list(modr1,modr2,modr3,modr4)
  names(modr) <- names(dat_wb_means)
  
  saveRDS(modr, "Data/120_modr_list.rds")

}

# Put in list and remove the single models
tic()
modr <- readRDS("Data/120_modr_list.rds")
toc()  

```

#### Get year effect data

-   Using 'plot_smooth' in R package 'itsadug'

```{r}
#| results: hide  

if (!"itsadug" %in% installed.packages())
  install.packages("itsadug")

library(itsadug)
plotdata_time_effect_list <- purrr::map(modr, plot_smooth, view = "year", 
                                        rug = F, 
                                        main = "GAM for time (random effect for waterbody removed)", 
                                        rm.ranef = TRUE)
...
plotdata_time_effect <- map_dfr(plotdata_time_effect_list, "fv",  .id = "modelrun")

ggplot(plotdata_time_effect, aes(year, fit, group = modelrun)) +
  geom_line(aes(color = modelrun)) +
  theme_bw()

ggplot(plotdata_time_effect, aes(year, fit, group = modelrun)) +
  geom_ribbon(aes(ymin = ll, ymax = ul), fill = "grey50", alpha = 0.3) +
  geom_line(aes(color = modelrun)) +
  theme_bw()

ggplot(plotdata_time_effect, aes(year, fit)) +
  geom_ribbon(aes(ymin = ll, ymax = ul), fill = "grey50", alpha = 0.3) +
  geom_line() +
  theme_bw() +
  facet_wrap(vars(modelrun))
