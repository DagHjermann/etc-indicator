...
# if(TRUE) {
  ...
  tic()
  mod1 <- map_dfr(1:10, get_fit_allyears_stoch, data = dat_wb_means[[1]])
  saveRDS(mod1, "Data/135_mod1.rds")
  mod2 <- map_dfr(1:10, get_fit_allyears_stoch, data = dat_wb_means[[2]])
  saveRDS(mod2, "Data/135_mod2.rds")
  mod3 <- map_dfr(1:10, get_fit_allyears_stoch, data = dat_wb_means[[3]])
  saveRDS(mod3, "Data/135_mod3.rds")
  mod4 <- map_dfr(1:10, get_fit_allyears_stoch, data = dat_wb_means[[4]])
...
set.seed(25)
wb_test <- mod_raw[[1]] %>% pull(waterbody) %>% unique() %>% sample(size = 12)


ggplot(mod_raw[[1]] %>% filter(waterbody %in% wb_test), aes(year, spatialfit)) +
  geom_point(shape = "-") +
  facet_wrap(vars(waterbody))

ggplot(mod_raw[[1]] %>% filter(waterbody %in% wb_test), aes(year, se.spatialfit)) +
  geom_point(shape = "-") +
  facet_wrap(vars(waterbody))

ggplot(mod_raw[[1]] %>% filter(waterbody %in% "FRDG330"), aes(year, spatialfit)) +
  geom_point(size = rel(0.5))

ggplot(mod_raw[[1]] %>% filter(waterbody %in% "FRDG330"), aes(year, se.spatialfit)) +
  geom_point(size = rel(0.5))

 
```

### Summarize per water body  

```{r}

get_summaryfit_allyears <- function(dat_fitted_values){
  dat_fitted_values %>%
    group_by(waterbody, year) %>%
    summarize(
      spatialfit_mean = mean(spatialfit),
      spatialfit_mean_sd = sd(spatialfit),
      spatialfit_se_mean = sqrt(mean(se.spatialfit^2)), .groups = "drop"
    ) %>% 
    as.data.frame()
}
...
# test <- get_summaryfit_allyears(mod_raw[[1]])  

mod <- purrr::map_dfr(mod_raw, get_summaryfit_allyears, .id = "modelrun")  
...
mod_raw[[1]] %>%
  group_by(repl, year) %>%
  summarize(
    yearfit_byrep_mean = mean(spatialfit_mean),
    yearfit_byrep_sd = sd(spatialfit_mean),
    yearfit_byrep_sd_mean = mean(spatialfit_se_mean), .groups = "drop"
  )
...
dat_means <- mod %>%
  ...
  summarize(
    yearfit_mean = mean(spatialfit_mean),
    yearfit_sd = sd(spatialfit_mean),
    yearfit_sd_mean = mean(spatialfit_se_mean), .groups = "drop"
  )
  

dat_means <- mod_raw[[1]] %>%
  group_by(modelrun, year, repl) %>%
  summarize()
