...
-   135 vs 130: this script does tries to use proper error propagation    
...
saveRDS(pdata_for_gam_allyrs, "Data/135_pdata_for_gam_allyrs.rds")

# if you want to read this directly:
# pdata_for_gam_allyrs <- readRDS("Data/135_pdata_for_gam_allyrs.rds")
...
df_wb <- dat %>%
  ...
  summarize(
    ...
    group_1_rows = length(site[sitegroup %in% 1]),
    group_2_rows = length(site[sitegroup %in% 2]),
    group_3_rows = length(site[sitegroup %in% 3]),
    x = mean(x),
    y = mean(y),
    .groups = "drop"
  )
...
data_for_plot <- df_wb %>%
  mutate(
    Waterbody_group = case_when(
      group_1 > 0 ~ "Includes group 1 sites",
      group_2 > 0 ~ "No group 1, but some group 2",
      group_3 > 0 ~ "No group 1 or 2, but group 3"),
    Waterbody_group = factor(Waterbody_group, 
                   levels = c("Includes group 1 sites",
                              "No group 1, but some group 2", 
                              "No group 1 or 2, but group 3")),
    Included_in_analysis = case_when(
      group_1 == 0 & group_2 == 0 ~ "Excluded",
      TRUE ~ "Included")
  ) %>%
  arrange(desc(Waterbody_group))

gg <- ggplot(data_for_plot, aes(x, y)) +
  geom_point(aes(fill = Waterbody_group), pch = 21) +
  geom_path(data = country_borders, aes(x,y,group = group)) +
  scale_fill_brewer("Waterbody group", palette = "RdYlBu", direction = -1) +
  coord_fixed() +
  theme_bw()
...
xtabs(~Waterbody_group, data_for_plot)

cat("\n--------------------------------\nNumber of sites (by water body group)\n--------------------------------\n")
xtabs(n_sites ~ Waterbody_group, data_for_plot)
cat("\n---------------\n- Number of rows (years * sites)\n---------------\n")
xtabs(n_rows ~ Waterbody_group, data_for_plot)

cat("\n--------------------------------\nNumber of group 1 sites (by water body group)\n--------------------------------\n")
xtabs(group_1 ~ Waterbody_group, data_for_plot)
cat("\n")
xtabs(group_1 ~ Included_in_analysis, data_for_plot)
cat("\n---------------\n- Number of rows (years * sites)\n---------------\n")
xtabs(group_1_rows ~ Waterbody_group, data_for_plot)
xtabs(group_1_rows ~ Included_in_analysis, data_for_plot)

cat("\n--------------------------------\nNumber of group 2 sites (by water body group)\n--------------------------------\n")
xtabs(group_2 ~ Waterbody_group, data_for_plot)
cat("\n")
xtabs(group_2 ~ Included_in_analysis, data_for_plot)
cat("\n---------------\n- Number of rows (years * sites)\n---------------\n")
xtabs(group_2_rows ~ Waterbody_group, data_for_plot)
xtabs(group_2_rows ~ Included_in_analysis, data_for_plot)

cat("\n--------------------------------\nNumber of group 3 sites (by water body group)\n--------------------------------\n")
xtabs(group_3 ~ Waterbody_group, data_for_plot)
cat("\n")
xtabs(group_3 ~ Included_in_analysis, data_for_plot)
cat("\n---------------\n- Number of rows (years * sites)\n---------------\n")
xtabs(group_3_rows ~ Waterbody_group, data_for_plot)
xtabs(group_3_rows ~ Included_in_analysis, data_for_plot)


```


### Statistics for the 4 model runs  

```{r}

cat("Run 1: 1992 onwards, group 1 - number of water bodies, sites and site-years\n")
...
df <- dat %>%
  ...
  summarize(
    ...
    group_1 = length(unique(site[sitegroup %in% 1])),
    group_2 = length(unique(site[sitegroup %in% 2])),
    group_3 = length(unique(site[sitegroup %in% 3])), .groups = "drop"
  ) %>%
  filter(group_1 > 0)
nrow(df)
sum(df$n_sites)
...
cat("--------------------------------------------\n")
cat("Run 2: 2000 onwards, group 1-3 - number of water bodies, sites and site-years\n")
...
cat("--------------------------------------------\n")
cat("Run 3: 2000 onwards, group 1-2 - number of water bodies, sites and site-years\n")

df <- dat %>%
  filter(sitegroup %in% 1:2) %>%
  filter(year >= 2000) %>%
  group_by(countryCode, waterbody) %>%
  summarize(
    n_rows = length(site),
...
cat("--------------------------------------------\n")
cat("Run 4: 2000 onwards, group 1 - number of water bodies, sites and site-years\n")
...
cat("Site by year table: \n")
table(dat_test$site, dat_test$year)

m1 <- gam(no3 ~ s(year, k = 3), data = dat_test)
...
dat_test_means <- dat_test %>%
  group_by(year) %>%
  summarize(no3 = mean(no3))

gg1 <- ggplot(dat_test, aes(x = year, y = no3)) +
  geom_point(data = dat_test_means, color = "black", shape = 1, size = rel(3)) +
  geom_point(aes(color = site_f)) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  # geom_point(data = dat_test_pred, color = "black") +
  scale_color_brewer("Site", palette = "Dark2") +
  coord_cartesian(ylim = c(0,100)) +
  labs(title = "Using means", y = "Observed NO3 conc.", x = "Year") + 
  theme_bw() +
  easy_remove_legend()
# gg1

gg2_with_legend <- ggplot(dat_test4, aes(x = year, y = fit)) +
  # geom_pointrange(aes(ymin = fit-2*se.fit, ymax = fit+2*se.fit, color = site_f), size = rel(1)) +
  geom_linerange(aes(ymin = fit-2*se.fit, ymax = fit+2*se.fit, color = site_f), size = rel(1)) +
  geom_point(aes(color = site_f)) +
  geom_point(data = dat_test4 %>% group_by(year) %>% summarize(fit = mean(fit)), 
             color = "black", shape = 1, size = rel(3)) +
  geom_line(data = dat_test4_mean, size = rel(1)) +
  scale_color_brewer("Site", palette = "Dark2") +
  coord_cartesian(ylim = c(0,100)) +
  labs(title = "Using a random site factor", y = "Expected NO3 conc.", x = "Year") + 
  theme_bw()
gg2 <- gg2_with_legend + easy_remove_legend()
legend <- cowplot::get_legend(gg2_with_legend)
# gg2 

gg <- cowplot::plot_grid(gg1, gg2, legend, nrow = 1, rel_widths = c(2,2,1))

ggsave("Figures/135_random_sie_example.png", gg, width = 9, height = 4, dpi = 200)
...
get_predictions <- function(model, data){
  ...
  pred_object <- predict(model, newdata = data_pred, se.fit = TRUE)
  data_pred$fit <- pred_object$fit
  data_pred$se.fit <- pred_object$se.fit
  data_pred
...
if (FALSE){
  ...
  df_pred <- run_gam_2("DEGB_DEMV_WP_WA_3", 1:3, years = 1992:2020)
  # yrs <- dat %>% filter(waterbody %in% "DEGB_DEST_SAL-GW-042") %>% pull(year)
  # predict(m1, data.frame(year = yrs))
...
if (rerun_gam_fitting1){
  ...
  get_wb_means <- function(result_orig, metadata_waterbody){
    ...
    bind_rows(result$result[ok]) %>%
      ...
      summarize(
        no3 = mean(fit), 
        no3_se = sqrt(mean(se.fit^2)), .groups = "drop") %>%
  ...
  saveRDS(dat_wb_bysite, "Data/135_dat_wb_bysite.rds")
  saveRDS(dat_wb_means, "Data/135_dat_wb_means.rds")
  # saveRDS(res_orig, "Data/135_res_orig.rds")
  
}

dat_wb_means <- readRDS("Data/135_dat_wb_means.rds")  
...
### Gam with random variation added to 'data'   

* Makes stochastic data based on the SE derived from each water body's model  
* Runs GAM  
* Makes data set with all water bodies and all years, including year x WB combinations that's not in the data    
* Predict results (NO3) for the data set using the given GAM  

```{r}

get_fit_allyears_stoch <- function(i, data){
  # Make stochastic data  
  dat_stoch <- data %>%
    mutate(no3_stoch = rnorm(n=length(no3), mean = no3, sd = no3_se))
  # GAM model
  model <- gam(no3_stoch ~ s(x, y, year, k=600), data = dat_stoch)
  # Data for prediction
...
# test <- get_fit_allyears_stoch(dat_wb_means[[1]])  

# tic()
# test2 <- map_dfr(1:2, get_fit_allyears_stoch, data = dat_wb_means[[1]])
# toc()  # 67 sec

```


### Run GAMs and retrieve 
...
# if(TRUE) {
  ...
  tic()
  mod1 <- map_dfr(repl_number, get_fit_allyears_stoch, data = dat_wb_means[[1]])
  saveRDS(mod1, "Data/135_mod1.rds")
  mod2 <- map_dfr(repl_number, get_fit_allyears_stoch, data = dat_wb_means[[2]])
  saveRDS(mod2, "Data/135_mod2.rds")
  set.seed(21)
  mod3 <- map_dfr(repl_number, get_fit_allyears_stoch, data = dat_wb_means[[3]])
  saveRDS(mod3, "Data/135_mod3.rds")
  mod4 <- map_dfr(repl_number, get_fit_allyears_stoch, data = dat_wb_means[[4]])
  saveRDS(mod4, "Data/135_mod4.rds")
  toc() # 4400 sec circa (73 min)

}

# Read saved data  
mod1 <- readRDS("Data/135_mod1.rds")
mod2 <- readRDS("Data/135_mod2.rds")
mod3 <- readRDS("Data/135_mod3.rds")
mod4 <- readRDS("Data/135_mod4.rds")

# Put in list and remove the single models
mod_raw <- list(mod1, mod2, mod3, mod4)
names(mod_raw) <- names(dat_wb_means)
...
#### Check some water bodies    
```{r}

# Some random water bodies

i <- 3

set.seed(25)
wb_test <- mod_raw[[i]] %>% pull(waterbody) %>% unique() %>% sample(size = 12)


ggplot(mod_raw[[i]] %>% filter(waterbody %in% wb_test), aes(year, spatialfit)) +
  geom_point(shape = "-") +
  facet_wrap(vars(waterbody))

ggplot(mod_raw[[i]] %>% filter(waterbody %in% wb_test), aes(year, se.spatialfit)) +
  geom_point(shape = "-") +
  facet_wrap(vars(waterbody))

ggplot(mod_raw[[i]] %>% filter(waterbody %in% "FRDG330"), aes(year, spatialfit)) +
  geom_point(size = rel(0.5))

ggplot(mod_raw[[i]] %>% filter(waterbody %in% "FRDG330"), aes(year, se.spatialfit)) +
  geom_point(size = rel(0.5))

```


### Summarize per year for each repl  

#### Test 
```{r}

df_by_year_repl <- mod_raw[[1]] %>% 
  group_by(repl, year) %>%
  summarize(
    no3_repl = mean(spatialfit),
    no3_repl_se = mean(se.spatialfit), .groups = "drop")

df_by_year <- df_by_year_repl %>% 
  group_by(year) %>%
  summarize(no3_mean = mean(no3_repl),
            no3_sd1 = sd(no3_repl),
            no3_sd2 = sqrt((sd(no3_repl))^2 + no3_repl_se), .groups = "drop")

ggplot(df_by_year_repl, aes(year, no3_repl)) +
  geom_point(color = "lightblue3") +
  geom_line(data = df_by_year, aes(y = no3_mean)) +
  geom_line(data = df_by_year, aes(y = no3_mean - 2*no3_sd1), linetype = "dashed") +
  geom_line(data = df_by_year, aes(y = no3_mean + 2*no3_sd1), linetype = "dashed") +
  theme_bw()
...
ggplot(df_by_year_repl, aes(year, no3_repl)) +
  geom_point(color = "lightblue3") +
  geom_line(data = df_by_year, aes(y = no3_mean)) +
  geom_line(data = df_by_year, aes(y = no3_mean - 2*no3_sd2), linetype = "dashed") +
  geom_line(data = df_by_year, aes(y = no3_mean + 2*no3_sd2), linetype = "dashed") +
  theme_bw()
...
df_by_year_repl <- map_dfr(
  mod_raw, 
  ~group_by(., repl, year) %>% 
    summarize(no3_repl = mean(spatialfit), .groups = "drop"),
  .id = "modelrun")

modelrun_order <- c("Since 1992, site group 1", "Since 2000, site group 1-3", "Since 2000, site group 1-2", "Since 2000, site group 1")

df_by_year <- df_by_year_repl %>%
  mutate(modelrun = factor(modelrun, levels = modelrun_order)) %>%
  group_by(modelrun, year) %>%
  summarize(no3_mean = mean(no3_repl),
            no3_sd = sd(no3_repl), .groups = "drop")

ggplot(df_by_year_repl, aes(year, no3_repl)) +
  geom_point(color = "lightblue3") +
  geom_line(data = df_by_year, aes(y = no3_mean)) +
  geom_line(data = df_by_year, aes(y = no3_mean - 2*no3_sd), linetype = "dashed") +
  geom_line(data = df_by_year, aes(y = no3_mean + 2*no3_sd), linetype = "dashed") +
  facet_wrap(vars(modelrun)) +
  theme_bw() +
  labs(x = "Year", y = "NO3 concentration")

gg <- ggplot(df_by_year, aes(year, no3_repl, color = modelrun)) +
  geom_line(aes(y = no3_mean)) +
  geom_line(aes(y = no3_mean - 2*no3_sd), linetype = "dashed") +
  geom_line(aes(y = no3_mean + 2*no3_sd), linetype = "dashed") +
  scale_color_brewer("Model run", palette = "Dark2") +
  theme_bw() +
  labs(x = "Year", y = "NO3 concentration")

ggsave("Figures/135_overall_temporal_change.png", gg, width = 8, height = 3.5, dpi = 200)

gg

```

## 9. Change 2000-2020 mapped  

### Trend by water body - function   
```{r}

get_change <- function(predictions_with_repl){
  
  df2020 <- predictions_with_repl %>% filter(year %in% 2020) %>% 
    rename(fit2020 = spatialfit, 
           se2020 = se.spatialfit) 
  df2000 <- predictions_with_repl %>% filter(year %in% 2000) %>% 
    rename(fit2000 = spatialfit, 
           se2000 = se.spatialfit)
  
  df_change <- left_join(
    df2020 %>% select(repl, waterbody, x, y, fit2020, se2020),
    df2000 %>% select(repl, waterbody, x, y, fit2000, se2000),
    by = c("repl", "waterbody", "x", "y")
  ) %>%
    mutate(
      change = fit2020 - fit2000,
      change_se = sqrt(se2020^2 + se2000^2)) %>%
    group_by(waterbody, x, y) %>%
    summarise(
      change_lo = mean(change) - 2*mean(change_se),
      change_up = mean(change) + 2*mean(change_se),
      .groups = "drop") %>%
    mutate(
      change_text = case_when(
        change_lo > 0 ~ "Increase",
        change_up < 0 ~ "Decrease",
        TRUE ~ "Inconclusive"),
      change_text = factor(change_text, 
                           levels = c("Increase", "Decrease", "Inconclusive"))
    )
  df_change
}

# test <- get_change(mod_raw[[1]])
# table(test$change_text)

```
### Trend by water body - calculate  

```{r}

df_change <- map_dfr(mod_raw, get_change, .id = "modelrun") %>%
  mutate(modelrun = factor(modelrun, levels = modelrun_order))
...
gg <- ggplot(df_change, aes(x,y)) +
  geom_path(data = country_borders, aes(x,y,group = group)) +
  geom_point(aes(color = change_text), size = rel(1)) +
  facet_grid(vars(modelrun), vars(change_text)) +
  coord_fixed() +
  theme_bw() +
  easy_remove_legend() +
  easy_remove_axes(what = c("text", "title", "ticks"))  

ggsave("Figures/135_change_by_waterbody.png", gg, width = 7, height = 9, dpi = 200)
...
tab <- df_change %>%
  count(modelrun, change_text) %>%
  group_by(modelrun)%>%
  mutate(tot = sum(n)) %>%
  ungroup() %>%
  mutate(percent = round(100*n/tot, 1))

tab

tab %>% 
  select(-n,-tot) %>% 
  tidyr::pivot_wider(names_from = change_text, values_from = percent) %>%
  knitr::kable()
